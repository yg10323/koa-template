...
const { without, isEqual } = require('lodash')

// 根据数据表生成对应的 sequelize model
const generateModel = async () => {
  const sequelize = seqInstance.getSequelizeInstance()
  const options = {
  ...
  if (sequelize) {
    const auto = new SequelizeAuto(sequelize, $consts['CONFIG/DB_USER'], $consts['CONFIG/DB_PASSWORD'], options)
    ...
    const tables = res[0]?.map(item => item.TABLE_NAME) || []
